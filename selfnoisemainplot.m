...
% plots the selected data in the axes.
    ...
    hsen2 = uicontrol('Style','text','String','Sensor 2 Type',...
    ...
    lowpasscon = uicontrol('Style','pushbutton',...
          'String','Low Pass',...
    ...
    align([hchan1,hsen1,hchan2,hsen2,hchan3,hsen3,hpopupsen1, ...
        ...
        starttime, endtime, hstime, hetime, lowpasscon],'Center','None');
   ...
   set([f,ha,hchan1,hsen1,hchan2,hsen2,hchan3,hsen3,hpopupsen1, ...
       hpopupsen2, hpopupsen3, hpopupdig1, hpopupdig2, hpopupdig3, ...
       hdig1, hdig2, hdig3, plotme, clearme, saveme, ...
       starttime, endtime, hstime, hetime, lowpasscon],...
   'Units','normalized');
   ...
   % Assign the GUI a name to appear in the window title.
    set(f,'Name','Self-Noise Estimate')
   ...
   % Make the GUI visible.
    ...
    function plot_Callback(~,~)
        ...
        stahand2=[];
        stahand3=[];
        if(~isempty(datafile1) && ~isempty(datafile2) && ~isempty(datafile3))
            [pspec1, stahand1, tseries1,ttime1, mabsvolt1, ...
                pspec2, stahand2, tseries2,ttime2, mabsvolt2, ...
                pspec3, stahand3, tseries3,ttime3, mabsvolt3, per, ...
                np1, np2, np3, ...
                ] = sleeman(datafile1, sen1,dig1, datafile2, sen2, ...
                dig2, datafile3, sen3, dig3, stimestr,etimestr, lowpass);
           
            thand1=stahand1(length(stahand1)-8:length(stahand1));
            stahand1=stahand1(1:length(stahand1)-8);
            subplot(10,6,[ 1 2 3 4 5 7 8 9 10 11 13 14 15 16 17 19 20 21 22 23 ] )
            phand1=semilogx(per,pspec1,'r:'); 
            
            hold on
            phand1n=semilogx(per,np1,'r');
            phand2=semilogx(per,pspec2,'b:');  
            phand2n=semilogx(per,np2,'b');
            thand3=stahand3(length(stahand3)-8:length(stahand3));
            stahand3=stahand3(1:length(stahand3)-8);
    
            phand3=semilogx(per,pspec3,'g:');  
       
            phand3n=semilogx(per,np3,'g');
        ...
        semilogx(nhnm(:,1),nhnm(:,2),'color','k','LineWidth',3);
        set(gca,'FontSize',14);
        title('Self-Noise Estimate','FontSize',14);
        xlabel('Period (s)','FontSize',14);
        ylabel('Power (dB)','FontSize',14);
        xlim([.01 1000]);
        ...
        if(length([phand1 phand2 phand3]) == 3);
            legend([phand1 phand1n phand2 phand2n phand3 phand3n p1], ...
                stahand1, [stahand1 ' Noise'], stahand2, [stahand2 ' Noise'] , ...
                stahand3,[stahand3 ' Noise'],'NLNM','Location','NorthWest');
        else
